...
int TicTacToe::easyMove() {
    ...
    for (int i = 0; i < 9; i++) {
        if (board[i] == EMPTY) {
            ...
            tempBoard[i] = PLAYER1; // AI makes the move

            std::vector<char> originalBoard = board;
            board = tempBoard;
            bool immediateWin = checkWin(PLAYER1);
            board = originalBoard;
            ...
            tempBoard = board;
            tempBoard[i] = PLAYER2; // Try opponent's move

            board = tempBoard;
            bool opponentWins = checkWin(PLAYER2);
            board = originalBoard;

            isBlockingMove.push_back(opponentWins);
        }
    }
