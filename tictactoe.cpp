...
#include <algorithm>
#include <limits>
#include <QSqlDatabase>   // Added for SQL
#include <QSqlQuery>      // Added for SQL
// ==================== Constructor ====================
...
void TicTacToe::gameOver(const QString &message, bool seriesOver) {
    ...
    if (!guestMode && seriesOver) {
        ...
        if (mode == 1) {
            saveMatchResult(winner, loser, result);
        } else if (mode == 2) {
            if (result == "Tie") {
                saveMatchResult("Player", "AI", "Tie");
            } else if (winner != "AI") {
                saveMatchResult(winner, "AI", "Win");
            } else {
                saveMatchResult("Player", "AI", "Defeat");
            }
        }
    }
...
void TicTacToe::saveMatchResult(const QString &winner, const QString &loser, const QString &result) {
    if (!db.isOpen()) return;
