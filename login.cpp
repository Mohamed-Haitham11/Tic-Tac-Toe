...
void TicTacToe::logout() {
    // Only save surrender if there's an active unfinished match
    if (isMatchUnfinished() && !guestMode && !moveHistory.empty()) {
        // Calculate BEFORE any score updates
        int actualGameNumber = player1Wins + player2Wins + ties + 1;

        QString surrenderingPlayer;
        QString winner;
        QString result;

        if (mode == 2) { // Player vs AI
            surrenderingPlayer = loggedInUser.isEmpty() ? player2Name : loggedInUser;
            winner = "AI";
            result = "🚪 Logout Surrender"; // Door emoji for logout surrender
        } else { // Player vs Player
            // Determine who is currently logged in and surrendering
            if (loggedInUser.isEmpty()) {
                surrenderingPlayer = (currentPlayer == PLAYER1) ? player1Name : player2Name;
            } else {
                surrenderingPlayer = loggedInUser;
            }

            // Winner is the other player
            if (surrenderingPlayer == player1Name) {
                winner = player2Name;
            } else {
                winner = player1Name;
            }
    ...
    // Reset game state to prevent carrying over data
    resetGameState();
...
// This function now ONLY handles the user confirmation.
void TicTacToe::deleteAccount() {
    if (loggedInUser.isEmpty()) {
        if (!isTestRun) QMessageBox::warning(this, "Delete Account", "You must be logged in to delete your account.");
        return;
    }
    ...
    if (!isTestRun) {
        ...
        reply = QMessageBox::Yes; // Automatically "click" Yes during tests
    }

    if (reply == QMessageBox::Yes) {
        performAccountDeletion(); // Call the new function to do the actual work.
    }
}

// This new function contains ONLY the database logic, which is now testable.
void TicTacToe::performAccountDeletion() {
    QSqlQuery query(this->db);
    ...
    query.bindValue(":username", loggedInUser);
    if (query.exec()) {
        if (!isTestRun) QMessageBox::information(this, "Account Deleted", "Your account and match history have been deleted.");
        logout();
    } else {
        if (!isTestRun) QMessageBox::critical(this, "Error", "Failed to delete account: " + query.lastError().text());
    }
}
...
void TicTacToe::replayNextMove() {
    ...
    if (checkWin(currentReplayPlayer, this->board)) {
...
bool TicTacToe::checkWin(char player, const std::vector<char>& targetBoard) { // <-- MODIFIED signature
    for (int i = 0; i < 3; i++) {
        if (targetBoard[i*3] == player && targetBoard[i*3+1] == player && targetBoard[i*3+2] == player) return true;
    }
    for (int i = 0; i < 3; i++) {
        if (targetBoard[i] == player && targetBoard[i+3] == player && targetBoard[i+6] == player) return true;
    }
    if (targetBoard[0] == player && targetBoard[4] == player && targetBoard[8] == player) return true;
    if (targetBoard[2] == player && targetBoard[4] == player && targetBoard[6] == player) return true;
    return false;
}

// In login.cpp
bool TicTacToe::checkTie(const std::vector<char>& targetBoard) { // <-- MODIFIED signature
    for (int i = 0; i < 9; i++) {
        if (targetBoard[i] == EMPTY) return false;
    }
    return true;
}

bool TicTacToe::isMatchUnfinished() {
   return !checkWin(PLAYER1, this->board) && !checkWin(PLAYER2, this->board) && !checkTie(this->board);
}
...
void TicTacToe::connectToDatabase(const QString& connectionName) {
...
void TicTacToe::saveIndividualGameWithNumber(const QString &winner, const QString &result, int gameNumber) {
    if (guestMode) return;
    ...
    query.bindValue(":game_number", gameNumber); // Use the passed game number
...
void TicTacToe::handleSurrender() {
    // Calculate current game number BEFORE updating any scores
    int actualGameNumber = player1Wins + player2Wins + ties + 1;
    ...
    // Determine who is surrendering
    if (mode == 2) { // Player vs AI
        surrenderingPlayer = player2Name;
        winner = "AI";
        result = "🏳️ Surrender"; // White flag emoji for surrender
    } else { // Player vs Player
        surrenderingPlayer = (currentPlayer == PLAYER1) ? player1Name : player2Name;
    ...
    // Save with the correct game number
    saveIndividualGameWithNumber(winner, result, actualGameNumber);

    QMessageBox::information(this, "Surrender",
                             QString("🏳️ %1 surrendered in Round %2! Series ended.")
                                 .arg(surrenderingPlayer)
                                 .arg(actualGameNumber));

    // IMMEDIATE EXIT - Reset everything and go back to mode selection
    resetGameState();
    backToModeSelection();
}
